...
public class ProgressBarShield extends Actor {
	...
	public ProgressBarShield(Texture textureBase, Texture textureStep, GamePlayScreen gPS) {
	...
	public void draw(Batch batch, float parentAlpha) {
		if (isVisible) {
			...
			for(int i=0; i<gPS.getgLL().getShieldPlayer(); i++) {
			...
			for(int i=0; i<gPS.getgLL().getShieldPlayer(); i++) {this.step[i].draw(batch);}
		}
	}
}
